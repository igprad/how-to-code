...
int main() {
    ...
    double diff_time = std::chrono::duration<double, std::milli>(end - start).count();

    double total = 0;
    for(int i=0; i < MAX; i++) {
        total+=result[i];
    }
    std::cout << "To verify all sum was correct, will sum all result elements with total: " << total << std::endl;
    std::cout << "Processing time for each row summarization: " << diff_time << " ms." << std::endl;
